...
import { ArrowLeft, CheckCircle2, XCircle, AlertCircle, Flag, List, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
interface QuizProps {
    ...
    startIndex: number;
    onExit: () => void;
    onUpdateProgress: (phase: string, correct: boolean, isFinished: boolean) => void;
}
...
export function Quiz({ phase, questions, startIndex, onExit, onUpdateProgress }: QuizProps) {
    const [currentIndex, setCurrentIndex] = useState(Math.min(startIndex, questions.length - 1));
    ...
    const [questionStates, setQuestionStates] = useState<Record<number, QuestionState>>(() => {
        ...
        for (let i = 0; i < questions.length; i++) {
            initial[i] = { answered: i < startIndex, correct: i < startIndex ? true : null, selectedOptions: [], flagged: false };
        }
        return initial;
    });

    // Active selection for current question (before submit)
    ...
    const correctCount = Object.values(questionStates).filter(s => s.correct === true).length;
    const flaggedCount = Object.values(questionStates).filter(s => s.flagged).length;

    const goToQuestion = (idx: number) => {
        setCurrentIndex(idx);
        const state = questionStates[idx];
        setSelectedOptions(state?.answered ? state.selectedOptions : []);
        setShowNav(false);
    };
    ...
    const handleOptionToggle = (optionPrefix: string) => {
        ...
        if (question.type === 'unique' || question.type === 'boolean') {
            ...
            setSelectedOptions(prev =>
                prev.includes(optionPrefix)
                    ? prev.filter(p => p !== optionPrefix)
                    : [...prev, optionPrefix]
            );
        }
    };
    ...
    const checkAnswer = () => {
        ...
        onUpdateProgress(phase, isAnsCorrect, isAllDone);
    };
    ...
    const toggleFlag = () => {
        setQuestionStates(prev => ({
            ...
            [currentIndex]: {
                ...prev[currentIndex],
                flagged: !prev[currentIndex].flagged
            }
        }));
    };
    ...
    const getFilteredQuestions = () => {
        ...
        switch (navFilter) {
            case 'flagged':
                return indices.filter(i => questionStates[i]?.flagged);
            case 'unanswered':
                return indices.filter(i => !questionStates[i]?.answered);
            case 'incorrect':
                return indices.filter(i => questionStates[i]?.correct === false);
            default:
                return indices;
        }
    };
    ...
    const isCorrect = currentState?.correct;
    const displayOptions = isAnswered ? (currentState?.selectedOptions || []) : selectedOptions;
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex">
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-y-auto p-2">
                        {getFilteredQuestions().map(idx => {
                            ...
                            let dotColor = 'bg-gray-300'; // unanswered
                            if (state?.answered && state.correct) dotColor = 'bg-green-500';
                            else if (state?.answered && !state.correct) dotColor = 'bg-red-500';

                            return (
                                <button
                                    key={idx}
                                    onClick={() => goToQuestion(idx)}
                                    className={`w-full text-left p-3 rounded-lg mb-1 flex items-start gap-3 transition-all text-sm group
                                        ${isCurrent ? 'bg-huawei-50 ring-1 ring-huawei-red/30' : 'hover:bg-slate-100'}
                                    `}
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className={`truncate ${isCurrent ? 'text-gray-900 font-medium' : 'text-gray-700'}`}>
                                            {q.text}
                                        </p>
                                    </div>
                                    {state?.flagged && (
                                        <Flag className="w-3.5 h-3.5 text-amber-500 fill-amber-500 flex-shrink-0 mt-0.5" />
                                    )}
                                </button>
                            );
                        })}
            ...
            {/* Overlay backdrop */}
            {showNav && (
                <div className="fixed inset-0 z-40 bg-black/20 backdrop-blur-sm" onClick={() => setShowNav(false)} />
            )}
            ...
            <div className="flex-1 p-4 md:p-6">
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button
                            onClick={toggleFlag}
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${currentState?.flagged
                                ? 'bg-amber-100 text-amber-700 ring-1 ring-amber-300'
                                : 'bg-white text-gray-500 border border-slate-300 hover:bg-amber-50 hover:text-amber-600'
                                }`}
                        >
                            <Flag className={`w-4 h-4 ${currentState?.flagged ? 'fill-amber-500' : ''}`} />
                            ...
                            {question.options.map((opt, i) => {
                                ...
                                if (isAnswered) {
                                    if (isActuallyCorrect) {
                                        styleClass = "border-green-500 bg-green-50 z-10";
                                ...
                                return (
                                    <div
                                        key={i}
                                        onClick={() => handleOptionToggle(rawPrefix)}
                        ...
                        <div className="mt-8 flex items-center justify-between">
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className={`flex items-center gap-1 px-4 py-2.5 bg-white border border-slate-300 rounded-lg text-sm font-medium transition-colors shadow-sm
                                        ${currentIndex === 0 ? 'opacity-40 cursor-not-allowed text-gray-400' : 'text-gray-700 hover:bg-slate-50'}`}
                                >
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-1 px-4 py-2.5 bg-white border border-slate-300 rounded-lg text-sm font-medium transition-colors shadow-sm
                                        ${currentIndex === questions.length - 1 ? 'opacity-40 cursor-not-allowed text-gray-400' : 'text-gray-700 hover:bg-slate-50'}`}
                                >
                        ...
                        {isAnswered && (
                            <div className={`mt-6 p-4 rounded-lg flex items-start ${isCorrect ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                ...
                                <div>
                                    ...
                                    {!isCorrect && (
                                        <p className="text-sm mt-1 opacity-90">The actual correct answer is: {question.correctAnswers.join(', ')}</p>
                                    )}
                                </div>
                            </div>
                        )}
                        ...
                        {questions.map((_, idx) => {
                            ...
                            return (
                                <button
                                    key={idx}
                                    onClick={() => goToQuestion(idx)}
                                    title={`Question ${idx + 1}${isFlagged ? ' (flagged)' : ''}${state?.answered ? (state.correct ? ' ✓' : ' ✗') : ''}`}
